...
export default function ProjectsPage(){
  ...
  const edit = (id:string) => {
    ...
    if(!p) return
    setEditingId(id); setName(p.name); setNotes(p.notes||'')
  }
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="flex items-center">
            ...
            {editingId && <button className="ml-2 px-3 py-2 rounded border" onClick={()=>{ setEditingId(undefined); setName(''); setNotes('') }}>Cancel</button>}
          </div>
        </div>
      </Card>
      ...
      <Card className="mt-4">
        ...
        <div className="overflow-x-auto">
          <table className="table-fixed min-w-[600px] w-full text-left">
            <thead className="text-sm text-slate-500"><tr><th>#</th><th>Name</th><th>Notes</th><th>Created</th><th>Actions</th></tr></thead>
            <tbody>
              {(projects||[]).map((p,i)=>(
