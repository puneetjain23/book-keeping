...
export default function FlatsPage() {
  ...
  const create = async () => {
    ...
    if (editingId) {
      await db.flats.update(editingId, {
        projectId,
        partyId,
        flatNo,
        areaSqft: area,
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div className="flex items-center">
            <div className="mr-2">
              Amount: <strong>{Number(area || 0) * Number(rate || 0)}</strong>
            </div>
            ...
            <tbody>
              {(flats || []).map((f, i) => (
                <tr key={f.id} className="border-t">
                  ...
                  <td className="py-2">
                    {parties?.find(p => p.id === f.partyId)?.name || f.partyId || '-'}
                  </td>
                  <td className="py-2">{f.areaSqft}</td>
                  <td className="py-2">{f.ratePerSqft}</td>
                  <td className="py-2">{f.amount}</td>
                  <td className="py-2">
                    <button className="mr-2 text-sm" onClick={() => edit(f.id)}>
                      Edit
                    </button>
                    <button className="text-sm text-red-600" onClick={() => remove(f.id)}>
                      Delete
                    </button>
                  </td>
                </tr>
