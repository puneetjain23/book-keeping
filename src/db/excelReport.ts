...
export async function generateExcelReport(filters: ReportFilters) {
  ...
  const flats = flatsAll.filter(f => !projectId || f.projectId === projectId)
                        .filter(f => !partyId || f.partyId === partyId);
  const transactions = transactionsAll.filter(t => !projectId || t.projectId === projectId)
  ...
  for (const f of flats) {
    ...
    const prev = partyMap.get(pid) || { name: parties.find(p => p.id === pid)?.name || 'Unknown', expected: 0, received: 0 };
  ...
  const summaryData = Array.from(partyMap.values()).map(p => ({
    Party: p.name,
  ...
  wsSummary.addRow([]);
  wsSummary.addRow(['Total Expected', 'Total Received', '% Expected', '% Collected', 'Outstanding']);
  wsSummary.addRow([totalExpected, totalReceived, completionPercent, percentCollected, totalOutstanding]);
  wsSummary.addRow([]);
  ...
  summaryData.forEach((row) => {
    const r = wsSummary.addRow([row.Party, row['Expected Amount'], row['Received Amount'], row['% Difference']/100, row['Balance Due']]);
    ...
    if (pct > 0.3) r.getCell(4).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFCDD2' } }; // red
    else if (pct > 0.1) r.getCell(4).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF9C4' } }; // yellow
